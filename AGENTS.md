...
- **Drift/update checks are throttled and fully decoupled from any AI coding tool's session lifecycle.** They may run passively on CLI invocation (default 2h, configurable) or on explicit commands (`imwel status`, `imwel sync`). Passive notices report only remote template commit changes for the current binding/proposal targets, not local edits. They must never hook into, intercept, or block an AI tool's own chat/agent session.
