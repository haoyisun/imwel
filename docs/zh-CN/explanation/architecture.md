...
- CLI 调用时检查当前 binding/proposal 引用 remote 的被动检查（默认间隔 2 小时；可用 `IMWEL_FETCH_THROTTLE_MS` 覆盖；本地编辑不触发更新提示）
...
- 上文用到的核心术语 → [术语词表](../explanation/glossary.md)
- Manifest 字段 → [Manifest 参考](../reference/manifest.md)
- 完整 CLI 参考 → [命令](../reference/commands.md)
