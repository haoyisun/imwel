...
`--project` 与 `--module` 至少要选中其一。在已绑定目录上重跑 `imwel init` 即**换绑**：整套选择（工具、模块、可写项目）会被替换，因此对既有 Artifact 的本地修改会被覆盖。非交互模式必须提供选择类 flags；缺必填参数时以退出码 **1** 失败。
...
新增的模块只安装其**必需** Artifact；之后请运行 `imwel sync` 拉取最新内容。要安装模块的 optional Artifact，请通过 `imwel init` 换绑。
...
将本地工具文件反向渲染为规范 Artifact，并向上游提案（默认分支 + PR/MR）。对每个有安装路径的绑定工具做反向渲染；规范正文冲突时推送失败。仅**可写项目**的 Artifact 才可推送——对只读模块的本地修改绝不会被推送。若要有意地回馈某个模块，请对该模块使用 [`imwel propose`](#imwel-propose-file)（走一次经审阅的 PR/MR），这是允许的。
