...
In a consumer binding, the CLI directs you to the template repo instead of reporting a fake clean result. See [Lint & quality bar](../how-to/lint-and-publish.md).
...
Force-refreshes and reports the remote template state for the current binding and pending proposals. A directory that has proposals but no binding is supported. When a binding exists, status also reports local drift and runs a **rule health** check over the managed rendered files (health findings never change the exit code):
...
1. **Binding summary** (when a binding exists)
...
2. **Remote target results**
   - `Binding <remote>/<branch>: up to date ...` or `updated from ...` compares the branch with the last successful sync. Run `imwel sync` when it is updated.
   - `Proposal <project> → <remote>/<branch>: up to date ...` or `updated from ...` compares the target template branch with the baseline recorded when proposal tracking was confirmed. Run `imwel propose` to review and refresh that tracking.
   - `Proposal ... cannot determine remote updates because its baseline is missing` identifies a tracking record created by an older imwel version. Run `imwel propose` to add a fresh baseline.
   - A failed target check is reported separately and makes status return non-zero; retry after correcting the Git/network problem.
3. **Local drift** (binding only)
   ...
   Remote updates and local edits are independent, so an updated binding and local-edit line can appear together.
4. **Rule health** (binding only)
...
|----------|-------------|
| `IMWEL_FETCH_THROTTLE_MS` | Override the passive fetch throttle (default 2h). Invalid values fall back to default. Ordinary commands check only remotes referenced by the current binding or pending proposals, and local edits do not trigger the remote-update notice. Per-remote throttle is not supported. `sync` / `status` / `propose` always force-refresh. |
...
- Consumer workflow → [Install a template](../how-to/consume-for-cursor.md)
- Author workflow → [Author a template](../how-to/create-template-repo.md)
- Safety defaults and Git model → [Architecture](../explanation/architecture.md)
