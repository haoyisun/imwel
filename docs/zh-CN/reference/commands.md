...
在消费侧 binding 目录中，CLI 会引导你到模板仓，而不是假装检查通过。见 [Lint 与质量条](../how-to/lint-and-publish.md)。
...
始终强制刷新，并报告当前 binding 与 pending proposal 的远程模板状态。只有 proposal、没有 binding 的目录也可使用。当 binding 存在时，status 还会报告本地漂移，并对受管的已渲染文件运行**规则健康**检查（健康发现不改变退出码）：
...
2. **远程目标结果**
   - `Binding <remote>/<branch>：已是最新` 或 `已从 ... 更新到 ...`，比较模板分支与上次成功同步。出现更新时运行 `imwel sync`。
   - `Proposal <project> → <remote>/<branch>：已是最新` 或 `已从 ... 更新到 ...`，比较目标模板分支与确认 proposal 追踪时记录的基线。出现更新时运行 `imwel propose` 检查并刷新追踪。
   - `Proposal ... 缺少基线，无法判断远程更新` 表示该追踪由旧版 imwel 创建。运行 `imwel propose` 补充新基线。
   - 目标检查失败会单独显示，并使 status 返回非零；修复 Git/网络问题后重试。
3. **本地漂移**（仅 binding）
   - `检测到本地手工修改：<paths>` 表示列出的受管路径与本地 history 中记录的状态不同。先检查这些编辑，决定保留、贡献还是丢弃，再用 `imwel sync` 协调变更。
   - `未检测到漂移。` 仅在远程提交未变且受管路径均无本地编辑时显示。此时无需操作。

   远程更新与本地编辑相互独立，因此 binding 更新与本地编辑提示可以同时出现。
4. **规则健康**（仅 binding）
...
需要快速离线查看本地记录的 binding 和贡献追踪状态时，使用 `imwel binding show`。它只读取本地元数据并检查路径是否存在，不 fetch，也不运行规则健康检查。需要立即检查远程时，使用 `imwel status`：它始终强制刷新 binding 与 proposal 目标；存在 binding 时还会检查本地漂移和规则健康。
...
| 变量 | 说明 |
|------|------|
| `IMWEL_FETCH_THROTTLE_MS` | 覆盖被动 fetch 节流（默认 2 小时）。非法值回退默认。普通命令只检查当前 binding 或 pending proposal 引用的 remote，本地编辑不会触发远程更新提示。尚不支持按远程独立节流。`sync` / `status` / `propose` 始终强制刷新。 |
...
- 安全默认与 Git 模型 → [架构](../explanation/architecture.md)
